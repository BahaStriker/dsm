...
switch($_GET['json']) {
    case "servers":
		...
		foreach($items as $item) {
            //if($item['userid'] !== $_SESSION['user_id']){
             //   break;
            //}
            ...
            if(!empty($latest)) {
                ...
                if(stripos($os, 'centos') !== false) { $results["data"][$i][3] = '<img src="template/images/centos.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'cloudlinux') !== false) { $results["data"][$i][3] = '<img src="template/images/cloudlinux.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'coreos') !== false) { $results["data"][$i][3] = '<img src="template/images/coreos.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'debian') !== false) { $results["data"][$i][3] = '<img src="template/images/debian.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'fedora') !== false) { $results["data"][$i][3] = '<img src="template/images/fedora.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'freebsd') !== false) { $results["data"][$i][3] = '<img src="template/images/freebsd.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'proxmox') !== false) { $results["data"][$i][3] = '<img src="template/images/proxmox.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'redhat') !== false) { $results["data"][$i][3] = '<img src="template/images/redhat.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'routeros') !== false) { $results["data"][$i][3] = '<img src="template/images/routeros.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'suse') !== false) { $results["data"][$i][3] = '<img src="template/images/suse.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'ubuntu') !== false) { $results["data"][$i][3] = '<img src="template/images/ubuntu.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                elseif(stripos($os, 'windows') !== false) { $results["data"][$i][3] = '<img src="template/images/windows.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                else { $results["data"][$i][3] = '<img src="template/images/other.png" data-toggle="tooltip" title="'.$os.'" alt="'.$os.'">'; }
                // CPU
                $results["data"][$i][4] = '
                ...
                // NET
                $results["data"][$i][8] = formatBytes($qstats['totalin']) . __('/s') . ' <i class="fa fa-long-arrow-down"></i><br>';
                $results["data"][$i][8] .= formatBytes($qstats['totalout']) . __('/s') . ' <i class="fa fa-long-arrow-up"></i>';
                // UPTIME DONUTS
                $results["data"][$i][9] = '
                ...
                ';
                // LAST SEEN
                $results["data"][$i][10] = smartDate($latest['timestamp']);
            } else {
                $results["data"][$i][3] = ""; //os
                $results["data"][$i][4] = ""; //cpu
                $results["data"][$i][5] = ""; //ram
                $results["data"][$i][6] = ""; //disk
                $results["data"][$i][7] = ""; //load
                $results["data"][$i][8] = ""; //net
                $results["data"][$i][9] = ""; //uptime
                $results["data"][$i][10] = __('-') . "<br>" . __('Never Seen'); //last seen
            }
			$results["data"][$i][11] = "<div class='pull-right'><div class='btn-group'>";
                $results["data"][$i][11] .= '<a href="?route=servers/manage-'.$item['type'].'&id='.$item['id'].'" class="btn btn-primary btn-flat btn-sm"><i class="fa fa-eye"></i></a>';
            	if(in_array("editServer",$perms))
                    $results["data"][$i][11] .= '<a href="#" onClick=\'showM("?modal=servers/edit&reroute=servers&routeid=&id='.$item['id'].'&section=");return false\'  class="btn btn-success btn-flat btn-sm"><i class="fa fa-edit"></i></a>';
            	if(in_array("deleteServer",$perms))
                    $results["data"][$i][11] .= '<a href="#" onClick=\'showM("?modal=servers/delete&reroute=servers&routeid=&id='.$item['id'].'&section=");return false\' class="btn btn-danger btn-flat btn-sm"><i class="fa fa-trash-o"></i></a>';
                //if(in_array("manageServer",$perms))
                    $results["data"][$i][11] .= '<a href="?route=manageServer&id='.$item['id'].'" class="btn  btn-flat btn-sm"><i class="fa fa-cloud"></i></a>';
			$results["data"][$i][11] .= "</div></div>";
			$i++;
		}
