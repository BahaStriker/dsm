            ...
            <div class="form-group">
                ...
                <select class="form-control select2 select2-hidden-accessible" id="type" name="type" style="width: 100%;" tabindex="-1" aria-hidden="true" required>
                    ...
                    <?php if($check['type'] == "tcp" || $check['type'] == "udp" || $check['type'] == "icmp") { ?>
                        <option value='offline' <?php if($alert['type'] == "offline") echo "selected"; ?> ><?php _e('Check Offline'); ?></option>
                        <option value='responsetime' <?php if($alert['type'] == "responsetime") echo "selected"; ?> ><?php _e('Response Time'); ?></option>
                    <?php } ?>

                    <?php if($check['type'] == "blacklist") { ?>
                        <option value='blacklisted' <?php if($alert['type'] == "blacklisted") echo "selected"; ?> ><?php _e('Listed In Blacklist'); ?></option>
                    <?php } ?>

                    <?php if($check['type'] == "dns") { ?>
                        <option value='dnsfailed' <?php if($alert['type'] == "dnsfailed") echo "selected"; ?> ><?php _e('DNS Lookup Failed'); ?></option>
                    <?php } ?>
...
<script type="text/javascript">
    ...
    <?php if($alert['type'] == "offline" || $alert['type'] == "blacklist" || $alert['type'] == "dns") { ?>
